...
type MaterialRow = { material_id: number; cantidad: string | number; porcentaje_merma?: string | number | null; nombre?: string; material?: { nombre?: string; unidad_consumo?: string } };
type LaborRow = { proceso_produccion_id: number; costo_unitario: string | number; orden?: number | null; nombre?: string; proceso?: { nombre?: string } };
...
export default function ServicioFormulaPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setService(formula?.data); setCosts(costsResponse.ok ? costData?.data ?? null : null); setMaterialRows((formula?.data?.materiales ?? []).map((row: MaterialRow) => ({ ...row, nombre: row.material?.nombre ?? row.nombre }))); setLaborRows((formula?.data?.mano_obra ?? []).map((row: LaborRow) => ({ ...row, nombre: row.proceso?.nombre ?? row.nombre })));
