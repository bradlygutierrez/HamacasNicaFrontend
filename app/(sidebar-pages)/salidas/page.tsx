...
import SalidasTable from '@/app/_components/salidas-table';
...
export default function SalidasPage() {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) =>
      `${row.producto} ${row.colores} ${row.usuario} ${row.fecha} ${row.ubicacion} ${row.estatus}`
        .toLowerCase()
        .includes(term)
    );
  }, [rows, searchTerm]);
  ...
  return (
    <div className="w-full max-w-full overflow-x-hidden bg-[#456f89] px-3 py-4 text-[#08264d] sm:px-8 sm:py-7">
      ...
      <SalidasTable rows={filteredRows} />
